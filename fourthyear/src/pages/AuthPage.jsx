...
export default function AuthPage() {
  ...
  const handleSubmit = (e) => {
    ...
    if (isLogin) {
      ...
      console.log('بيانات إنشاء الحساب:', { fullName, email, password })
    }
      ...
      <div style={{ width: '100%', maxWidth: '900px', display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: '560px', borderRadius: 'var(--radius-xl)', overflow: 'hidden', boxShadow: '0 30px 80px rgba(0,0,0,.6)', border: '1px solid rgba(74,144,226,.2)', position: 'relative', zIndex: 1 }} className="auth-split">
        ...
        <div className="auth-mobile-tabs d-none d-show-mobile" style={{ gridColumn: '1 / -1', background: 'var(--navy-800)', padding: '12px' }}>
          <div style={{ display: 'flex', background: 'rgba(255,255,255,.05)', borderRadius: '50px', padding: '4px' }}>
            <button onClick={() => handleTabChange(true)} style={{ flex: 1, padding: '10px', borderRadius: '50px', background: isLogin ? 'var(--accent)' : 'transparent', color: isLogin ? '#fff' : 'var(--gray-400)', fontWeight: 700 }}>تسجيل الدخول</button>
            <button onClick={() => handleTabChange(false)} style={{ flex: 1, padding: '10px', borderRadius: '50px', background: !isLogin ? 'var(--accent)' : 'transparent', color: !isLogin ? '#fff' : 'var(--gray-400)', fontWeight: 700 }}>إنشاء حساب</button>
          </div>
        </div>
          ...
          <AnimatePresence mode="wait">
            {isLogin ? (
              <motion.div key="info-login" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} style={{ position: 'relative', zIndex: 1 }}>
                ...
                <button onClick={() => handleTabChange(false)} className="btn btn-outline" style={{ direction: 'rtl' }}>إنشاء حساب</button>
              </motion.div>
            ) : (
              <motion.div key="info-reg" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} style={{ position: 'relative', zIndex: 1 }}>
                ...
                <button onClick={() => handleTabChange(true)} className="btn btn-outline" style={{ direction: 'rtl' }}>تسجيل الدخول</button>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
